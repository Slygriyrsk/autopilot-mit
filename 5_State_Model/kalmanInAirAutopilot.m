...
for k = 1:N
    ...
    %estimated
    x_pred = Ad_gnd * x_hat + Bd_gnd * u_vec;
    P_pred = Ad_gnd * P * Ad_gnd' + Q_kf;

    %residual/innovation
    y_tilde = z - C_gnd * x_pred; %disp(y_tilde(1)); to get the kalman residual
    buf_ptr = buf_ptr + 1;
    if buf_ptr > innov_window
        buf_ptr = 1;
    end
    ...
    emp_innov_cov = cov(recent.');

    if any(isnan(emp_innov_cov),'all')
        emp_innov_cov = diag(var(recent,0,2)); % fallback per-state var on columns
    end
    
    S_theory = C_gnd * P_pred * C_gnd' + R_kf;
    gamma = 0.01; % adaptation rate (tune)
    R_adj = R_kf;

    if trace(emp_innov_cov) > trace(S_theory)
        R_new = alpha_R * R_kf + (1-alpha_R) * (emp_innov_cov - C_gnd*P_pred*C_gnd');
        R_new = (R_new + R_new')/2;
        %R_min=0.1*R_original; %constrain the allowed R to be not less than 10%
        R_new = diag(max(diag(R_new), 1e-12)); % keep diagonal for stability
        %R_new = diag(max(diag(R_new), diag(R_min)));
        R_kf = R_new;
    else
        R_kf = alpha_R * R_kf + (1-alpha_R) * R_original;
    end

    S = C_gnd * P_pred * C_gnd' + R_kf;
    K = P_pred * C_gnd' / S; % Kalman gain

    % Update
    x_hat = x_pred + K * y_tilde;
    P = (eye(size(P)) - K * C_gnd) * P_pred;
    
    K_full_store(:,:,k) = K;
    Krownorm_store(:,k) = vecnorm(K,2,2);
    gust_store(:,k)=gust;
    x_store(:,k) = X;
    delta_store(k) = delta;
    ...
    k_store(:,k) = diag(K); %other than diag all are 0's only
    k_colalt(:,k)=K(:,5);
    k_rowalt(k,:)=K(5,:);
    x_hat_store(:,k) = x_hat;
