...
%i will make modes for manual and autopilot control using 0: Manual, 
% 1:Autopilot
map1=dictionary([0,1],["Manual", "Autopilot"]);
cmd_mode=0;
...
%NOTE:one thing to keep in mind while in manual mode the U0 should be -ve and in
%autopilot mode, it should be +ve
A_aug = [A, zeros(4,1);
         0, -1, 0, -U0, 0];
...
Q_kf = diag([1e-13, 1e-13, 1e-14, 1e-14, 1e-11]);
...
R_kf = diag([ (2)^2, (2)^2, (1)^2, (1e-2)^2, (2)^2 ]);
...
for k = 1:N
    ...
    theta_ref = x_hat(4); vel_ref = x_hat(1); alt_ref = x_hat(5);

    if isKey(map1, cmd_mode)
        cmd_str = map1(cmd_mode);
        if strcmp(cmd_str, 'Manual')
            delta = theta_doublet_ref; 
            thrust = vel_doublet_ref; 

            theta_ref=x_hat(4);
            vel_ref=x_hat(1);
            alt_ref=x_hat(5);
            %in manual mode we must reset the integrators to prevent
            %wind-up when switching back to autopilot
            int_theta = 0; int_alt = 0; int_vel = 0;
            
        elseif strcmp(cmd_str, 'Autopilot')
            if isKey(map, current_mode)
                mode_str = map(current_mode);
                %outer loop control (PID)
    ...
    %adding constraints to finalize the sign
    if ~exist('thrust','var'), thrust = 0; end
    u_vec = [delta; thrust];
